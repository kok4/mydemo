...
class User():
    ...
    def login(self):
        if self.cfg["room"]["server_addr"] != "":
            return True  # 不连登录服，直接房间服
    ...
    def req_room(self):
        cfg_room_server_addr = self.cfg["room"]["server_addr"]
        if cfg_room_server_addr != "":
            print("Using configured room server addr: ", cfg_room_server_addr)
            self.room_server_addr = cfg_room_server_addr
            self.room_server_key = "room_server_key_debug"
            return True
